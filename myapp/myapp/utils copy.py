...
def get_credentials():
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name('static/credentials/ffv-iaa-myaccount.json', scope)
    return creds
...
def process_uploaded_file_iaa(uploaded_file, request):
    try:
        if uploaded_file.name.endswith('.xlsx'):
            df = pd.read_excel(uploaded_file, dtype='str')
        elif uploaded_file.name.endswith('.csv'):
...
def process_uploaded_file_vendor(uploaded_file, request):
    try:
        if uploaded_file.name.endswith('.xlsx'):
            df = pd.read_excel(uploaded_file, dtype='str')
        elif uploaded_file.name.endswith('.csv'):
